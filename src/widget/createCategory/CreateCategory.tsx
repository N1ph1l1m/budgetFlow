...
interface ICreateMessage {
  ...
  async  function sendCreateCategoryRequest(name: string, transaction: string, icon: File){
    ...
    useEffect(()=>{console.log(selectTransaction)},[selectTransaction])
  ...
  return (
    <div className={styles.wrapCreateCategory}>
      <header className={styles.headeWrap}>
        ...
        <select onChange={selectTypeTransaction} className={styles.selectTypeTransaction} id="selectTransaction">
          <option value="-">-----------</option>
        ...
        <input onChange={selectFile} id="inputFile" type="file" accept="image/png" />

        <button type="submit" className={styles.createCategory}>Создать</button>
        </form>
      </div>
  );
};
